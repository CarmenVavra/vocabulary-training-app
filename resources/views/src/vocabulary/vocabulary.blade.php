    ...
    <div class="container">
      <table id="tblVocs" class="table table-striped table-dark table-hover">
        ...
        <tbody>
           ...
           {{--  @php dd($vocabulary)@endphp --}}

          <tr>
            <td class="language">{{ $vocabulary->vocabularies[0]->name }}</td> {{-- Muttersprache --}}
            <td class="language">{{ $vocabulary->name }}</td> {{-- Fremdsprache --}}
            <td>
              <a href="{{ route('vocabulary.edit', $vocabulary->id/* [$vocabulary->vocabularies[0]->id, $vocabulary->id] */) }}" data-toggle="modal" data-target="#overlay-edit"><button type="button" class="btn btn-info btn-sm btn-edit">
            ...
            <td>
                <form action="{{ route('vocabulary.delete', $vocabulary->id /* [$vocabulary->vocabularies[0]->id,  ]*/ ) }}" method="post">
                    @csrf
                    @method('delete')
